...
update_status ModulePhysics::PostUpdate()
{
	...
	if(App->input->GetKey(SDL_SCANCODE_3) == KEY_DOWN)
	{
		...
		for (int i = 0; i < 12;i++)
		{
			vec[i] = {PIXEL_TO_METERS(points[2 * i]), PIXEL_TO_METERS(points[2 * i + 1])};
	...
	if (App->input->GetKey(SDL_SCANCODE_4) == KEY_DOWN)
	{
		...
		for (int i = 0; i < 41; i++)
		{
			vec[i] = { PIXEL_TO_METERS(rick_head[2 * i]), PIXEL_TO_METERS(rick_head[2 * i + 1]) };
